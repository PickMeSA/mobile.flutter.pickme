...
class _LocationPageState extends BasePageState<LocationPage, LocationBloc> {
        ...
        if(state is LocationRemoteSubmitLocationState && state.dataState == DataState.success){
         ...
         if(state.profileEntity!.description!.isEmpty) {
           ...
           if(profileEntity != null){
             context.router.push(FinalDetailsRoute(profileEntity: state.profileEntity!));
           }else{
  ...
  OTPLocationEntity getLocation(PickResult result){
    return OTPLocationEntity(address: result.formattedAddress, latitude: result.geometry?.location.lat, longitude: result.geometry?.location.lng);
  }
